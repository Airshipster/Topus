...
async function sendStarsInvoice(env: Env, project: Project, chatId: number, userId: string, months: number): Promise<void> {
  ...
  const payload: TelegramPayload = {
    ...
    description: recurring
      ? `Доступ к личной ленте за ${amount} звёзд в месяц. Telegram будет продлевать подписку автоматически. Рекуррентный платёж идёт по последней оплаченной вами сумме.`
