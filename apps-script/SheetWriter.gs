function appendPushEvent_(timestamp, videoId, channelId, rawXml) {
  ...
  if (columns['Timestamp GMT+4']) {
    sheet.getRange(nextRow, columns['Timestamp GMT+4']).setNumberFormat('yyyy-mm-dd hh:mm:ss');
  }
...
function ensurePushEventsHeader_(sheet) {
  ...
  var columns = pushEventsHeaderMap_(sheet);
  if (columns['Timestamp GMT+4']) {
    sheet.getRange(2, columns['Timestamp GMT+4'], Math.max(sheet.getMaxRows() - 1, 1), 1)
